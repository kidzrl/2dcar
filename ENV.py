...
class CarEnv(object):
    ...
    map_size_x = 1000
    map_size_y = 1000
    viewer_xy = (map_size_x, map_size_y)
    sensor_max = 260.
    start_point = [450, 300]
    speed = 50.
    dt = 0.1
    ...
    car_long = 40
    car_width = 20
    grid_num = int(map_size_x/car_long)  #each axis toatle grid num
    grid_size = map_size_x/grid_num      #each grid size
    grid_class = np.arange(0, grid_num*grid_num).reshape(grid_num,grid_num)
    ...
    def __init__(self, discrete_action=True):
        ...
        self.terminal = False
        # node1 (x, y, r, w, l),
        self.car_info = np.array([0, 0, 0, self.car_width, self.car_long], dtype=np.float64)   # car coordination
        self.a = np.array([0], dtype=np.float64)
    ...
    def step(self, action):
        ...
        # mindis = np.min(self.sensor_info[:, 0].flatten())
        # if self.terminal:
        #     r = self.punishment
        # elif (self.terminal == False) and (mindis < (self.car_info[-1]/2 + self.plus)):
        #     # r = (1/self.plus*(mindis - self.car_info[-1]/2) -1) * 5
        #     r = (-self.punishment/self.plus)*(mindis - self.car_info[-1]/2) + self.punishment
        # else:
        #     r = 0
        return s, r, self.terminal

    def step_transform(self, action):
        if self.is_discrete_action:
            action = self.actions[action]
        else:
            action = np.clip(action, *self.action_bound)[0]
        self.car_info[2] += action * np.pi/2  # max r = 6 degree
        self.car_info[:2] = self.car_info[:2] + \
                            self.speed * self.dt * np.array([np.cos(self.car_info[2]), np.sin(self.car_info[2])])

        self._update_sensor()
        # s = self._get_state()
        class_y = int(self.car_info[1]/self.grid_size)
        class_x = int(self.car_info[0] / self.grid_size)
        s = self.grid_class[class_x, class_y]
        self.a[0] = s
        # r = -1 if self.terminal else 1
        mindis = np.min(self.sensor_info[:, 0])
        if self.terminal:
            r = self.punishment
        elif (self.terminal == False) and (mindis < (self.car_info[-1]/2 + self.plus)):
            r = (1/20*(mindis - self.car_info[-1]/2) -1) * 5
            #r = (-self.punishment/self.plus)*(mindis - self.car_info[-1]/2) + self.punishment
        else:
            r = 0
        return self.a, r, self.terminal
    ...
    def _update_sensor(self):
        ...
        for si in range(len(self.sensor_info)):
            ...
            self.distance = np.min(possible_sensor_distance)
            distance_index = np.argmin(possible_sensor_distance)
            self.sensor_info[si, 0] = self.distance
            self.sensor_info[si, -2:] = possible_intersections[distance_index]
            if self.distance < self.car_info[-1]/2:
                self.terminal = True
